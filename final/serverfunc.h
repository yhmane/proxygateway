...
void setServmessage2(UB2 msgtype, *ipmsg, dgt_auth_res_msg *auth_res)                                   //set msgtype2                   
{
        auth_res->msg_type=AUTH_RES_MSG ;
        auth_res->rtn_len=strlen(ipmsg);
        memcpy(auth_res->rtn_msg, ipmsg, strlen(ipmsg)+1);
}
